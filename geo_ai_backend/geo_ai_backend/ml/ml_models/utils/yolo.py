...
def postprocess_yolo(preds: List[np.ndarray],
    ...
    for i, pred in enumerate(p):
        ...
        bboxes = find_bboxes(pred, img_hw, orig_img_hw)
        result = [bboxes]

        if preds_have_masks:
            masks = find_masks(i, proto, pred, img_hw, orig_img_hw)
            result.append(masks)
...
def find_masks(
    ...
    for i in range(masks.shape[2]):
        orig_sized_masks.append(
            np.expand_dims(cv2.resize(masks[:, :, i], orig_img_hw[::-1]), axis=0))
...
class LetterBox:
    ...
    def __init__(self, new_shape=(640, 640), auto=False, scaleFill=False, scaleup=True,
                 stride=32):
...
def non_max_suppression(
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if multi_label:
            ...
            x = np.concatenate((box[i], x[i, 4 + j, None], j[:, None].float(), mask[i]),
                               1)
...
def process_mask(protos, masks_in, bboxes, shape, upsample=False):
    ...
    ih, iw = shape
    masks = sigmoid(masks_in @ protos.astype(np.float32).reshape(c, -1)).reshape(-1, mh,
                                                                                 mw)  # CHW
